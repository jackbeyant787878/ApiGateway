...
```mermaid
flowchart TD
    A[客户端/浏览器] -- 1.发起请求 --> B[ASP.NET Core 中间件管道]
    B -- 预处理：跨域/权限/校验 --> C[YARP 核心转发中间件]
    C -- 2.路由匹配目标Cluster微服务 --> D[YarpPollyHandler 拦截入口]
    
    D -- 3.获取服务单例Policy --> E[Polly 洋葱圈策略链外层→内层：Fallback→Breaker→Retry]
    
    %% 分支1：服务正常，全链路放行
    E -- 正常无异常 --> F[SocketsHttpHandler 底层网络处理器]
    F -- 4.发起下游真实请求 --> G[下游微服务]
    G -- 200OK 正常响应 --> H[原路逐级返回响应至客户端]
    
    %% 分支2：下游偶发抖动，触发重试
    E -- 穿透Fallback、Breaker --> I[Retry重试策略拦截]
    I -- 捕获503/超时/网络抖动 --> J[打印重试日志+TraceId]
    J -- 网关内部延时重试 --> K{重试是否成功?}
    K -- 成功 --> H
    K -- 失败 --> L[异常向上冒泡]
    
    %% 分支3：重试耗尽，触发熔断+降级
    L -- 多次重试全部失败 --> M[Breaker熔断策略拦截]
    M -- 错误率超标 --> N[熔断器状态 Closed→Open]
    N -- 打印熔断告警日志+封禁30秒 --> O[拦截后续所有请求,跳过网络调用]
    O -- 抛出熔断异常 --> P[Fallback降级策略捕获异常]
    P -- 执行降级逻辑 --> Q[返回自定义200+友好JSON响应]
    Q --> H```
